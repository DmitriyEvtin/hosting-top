#!/usr/bin/env node

/**
 * Скрипт для проверки и применения миграций базы данных в production
 * Помогает диагностировать и исправлять проблемы с миграциями
 */

import { execSync } from "child_process";

function runCommand(command, options = {}) {
  try {
    const result = execSync(command, {
      encoding: "utf8",
      stdio: "pipe",
      ...options,
    });
    return { success: true, output: result };
  } catch (error) {
    return {
      success: false,
      error: error.message,
      output: error.stdout || "",
      stderr: error.stderr || "",
    };
  }
}

function checkDatabaseConnection() {
  console.warn("🔍 Проверка подключения к базе данных...");

  const result = runCommand("npx prisma db execute --stdin", {
    input: "SELECT 1 as test;",
  });

  if (result.success) {
    console.warn("✅ Подключение к базе данных успешно");
    return true;
  } else {
    console.warn("❌ Ошибка подключения к базе данных:");
    console.warn(result.error);
    return false;
  }
}

function checkMigrationStatus() {
  console.warn("\n📊 Проверка статуса миграций...");

  const result = runCommand("npx prisma migrate status");

  if (result.success) {
    console.warn("✅ Статус миграций:");
    console.warn(result.output);
    return true;
  } else {
    console.warn("❌ Ошибка при проверке статуса миграций:");
    console.warn(result.error);
    return false;
  }
}

function checkTablesExist() {
  console.warn("\n🗃️  Проверка существования таблиц...");

  const tables = ["users", "categories", "products", "parsing_sessions"];
  const missingTables = [];

  for (const table of tables) {
    const result = runCommand(`npx prisma db execute --stdin`, {
      input: `SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = '${table}');`,
    });

    if (result.success) {
      const exists =
        result.output.includes("t") || result.output.includes("true");
      if (exists) {
        console.warn(`✅ Таблица ${table} существует`);
      } else {
        console.warn(`❌ Таблица ${table} НЕ существует`);
        missingTables.push(table);
      }
    } else {
      console.warn(`❌ Ошибка при проверке таблицы ${table}:`);
      console.warn(result.error);
      missingTables.push(table);
    }
  }

  return missingTables;
}

function applyMigrations() {
  console.warn("\n🚀 Применение миграций...");

  const result = runCommand("npx prisma migrate deploy");

  if (result.success) {
    console.warn("✅ Миграции успешно применены:");
    console.warn(result.output);
    return true;
  } else {
    console.warn("❌ Ошибка при применении миграций:");
    console.warn(result.error);
    console.warn("STDOUT:", result.output);
    console.warn("STDERR:", result.stderr);
    return false;
  }
}

function generatePrismaClient() {
  console.warn("\n🔧 Генерация Prisma клиента...");

  const result = runCommand("npx prisma generate");

  if (result.success) {
    console.warn("✅ Prisma клиент успешно сгенерирован");
    return true;
  } else {
    console.warn("❌ Ошибка при генерации Prisma клиента:");
    console.warn(result.error);
    return false;
  }
}

function checkDatabaseHealth() {
  console.warn("\n🏥 Проверка здоровья базы данных...");

  const result = runCommand("npx prisma db execute --stdin", {
    input: `
      SELECT 
        'users' as table_name, COUNT(*) as count FROM users
      UNION ALL
      SELECT 
        'categories' as table_name, COUNT(*) as count FROM categories
      UNION ALL
      SELECT 
        'products' as table_name, COUNT(*) as count FROM products
      UNION ALL
      SELECT 
        'parsing_sessions' as table_name, COUNT(*) as count FROM parsing_sessions;
    `,
  });

  if (result.success) {
    console.warn("✅ Статистика таблиц:");
    console.warn(result.output);
    return true;
  } else {
    console.warn("❌ Ошибка при проверке статистики:");
    console.warn(result.error);
    return false;
  }
}

function main() {
  console.warn("🔧 Проверка и исправление миграций базы данных\n");

  // Проверка подключения
  if (!checkDatabaseConnection()) {
    console.warn("\n❌ Не удалось подключиться к базе данных");
    console.warn("💡 Решение:");
    console.warn("1. Проверьте переменную DATABASE_URL");
    console.warn("2. Убедитесь, что база данных запущена");
    console.warn("3. Проверьте права доступа к базе данных");
    process.exit(1);
  }

  // Проверка статуса миграций
  if (!checkMigrationStatus()) {
    console.warn("\n⚠️  Не удалось проверить статус миграций");
  }

  // Проверка существования таблиц
  const missingTables = checkTablesExist();

  if (missingTables.length > 0) {
    console.warn(`\n❌ Отсутствуют таблицы: ${missingTables.join(", ")}`);
    console.warn("\n🔧 Попытка исправления...");

    // Генерация Prisma клиента
    if (!generatePrismaClient()) {
      console.warn("\n❌ Не удалось сгенерировать Prisma клиент");
      process.exit(1);
    }

    // Применение миграций
    if (!applyMigrations()) {
      console.warn("\n❌ Не удалось применить миграции");
      console.warn("\n💡 Ручное решение:");
      console.warn("1. Запустите: make db-migrate");
      console.warn(
        "2. Или: docker compose -f docker-compose.prod.yml exec app npx prisma migrate deploy"
      );
      process.exit(1);
    }

    // Повторная проверка таблиц
    console.warn("\n🔄 Повторная проверка таблиц...");
    const stillMissing = checkTablesExist();

    if (stillMissing.length > 0) {
      console.warn(
        `\n❌ Все еще отсутствуют таблицы: ${stillMissing.join(", ")}`
      );
      console.warn("\n💡 Возможные решения:");
      console.warn("1. Проверьте права доступа к базе данных");
      console.warn("2. Убедитесь, что миграции корректны");
      console.warn("3. Попробуйте сбросить и пересоздать базу данных");
      process.exit(1);
    }
  }

  // Финальная проверка здоровья
  if (checkDatabaseHealth()) {
    console.warn("\n✅ База данных в порядке");
    console.warn("🚀 Приложение готово к работе");
  } else {
    console.warn("\n⚠️  Не удалось проверить статистику таблиц");
  }
}

// Запуск проверки
if (import.meta.url === `file://${process.argv[1]}`) {
  main();
}

export {
  applyMigrations,
  checkDatabaseConnection,
  checkDatabaseHealth,
  checkMigrationStatus,
  checkTablesExist,
  generatePrismaClient,
};
