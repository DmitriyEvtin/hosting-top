...
test.describe('Catalog', () => {
  ...
  test('user can filter products by category', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="filtered-products"]')).toBeVisible();
  });
});
```
...
test.describe('Admin Panel', () => {
  ...
  test('admin can start parsing', async ({ page }) => {
    await page.goto('/admin/parsing');
    
    // Запускаем парсинг
    await page.click('[data-testid="start-parsing-button"]');
    
    // Проверяем статус парсинга
    await expect(page.locator('[data-testid="parsing-status"]')).toContainText('Запущен');
  });
});
```
...
### Unit тесты парсера
```typescript
// tests/unit/parsing/bvb-alyans-parser.test.ts
import { BvbAlyansParser } from '../../src/features/parsing/api/bvb-alyans-parser';

describe('BvbAlyansParser', () => {
  let parser: BvbAlyansParser;

  beforeEach(() => {
    parser = new BvbAlyansParser();
  });
  ...
  it('handles parsing errors gracefully', async () => {
    // Мокаем ошибку сети
    jest.spyOn(parser, 'parseCategories').mockRejectedValue(new Error('Network error'));
    
    await expect(parser.parseCategories()).rejects.toThrow('Network error');
  });
});
```

### Integration тесты парсинга
```typescript
// tests/integration/parsing/parsing-workflow.test.ts
import { ParsingService } from '../../src/features/parsing/api/parsing-service';

describe('Parsing Workflow', () => {
  it('completes full parsing cycle', async () => {
    const parsingService = new ParsingService();
    
    const session = await parsingService.startParsing({
      type: 'full',
    ...
    await parsingService.waitForCompletion(session.id);
    
    const finalSession = await parsingService.getSession(session.id);
...
export const mockS3Service = {
  uploadImage: jest.fn().mockResolvedValue('https://s3.amazonaws.com/bucket/image.jpg'),
  deleteImage: jest.fn().mockResolvedValue(undefined),
  getImageUrl: jest.fn().mockReturnValue('https://s3.amazonaws.com/bucket/image.jpg'),
};
```
