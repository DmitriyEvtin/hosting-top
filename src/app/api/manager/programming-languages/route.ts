...
/**
 * Схема валидации для создания ProgrammingLanguage
 */
const ProgrammingLanguageCreateSchema = z.object({
  name: z.string().min(1, "Название обязательно").max(255, "Название слишком длинное"),
});

/**
 * Схема валидации для query параметров списка ProgrammingLanguage
 */
const ProgrammingLanguageQuerySchema = z.object({
...
/**
 * GET /api/manager/programming-languages - Получить список ProgrammingLanguage с поиском и сортировкой
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const validatedQuery = ProgrammingLanguageQuerySchema.parse(queryParams);
    ...
    // Получаем ProgrammingLanguage с пагинацией
    const [programmingLanguages, total] = await Promise.all([
...
/**
 * POST /api/manager/programming-languages - Создать новый ProgrammingLanguage
 */
export async function POST(request: NextRequest) {
  try {
    ...
    const validatedData = ProgrammingLanguageCreateSchema.parse(body);
    ...
    return NextResponse.json(
      {
        message: "ProgrammingLanguage успешно создан",
        programmingLanguage,
      },
      { status: 201 }
    );
  } catch (error) {
    console.error("Ошибка создания ProgrammingLanguage:", error);
