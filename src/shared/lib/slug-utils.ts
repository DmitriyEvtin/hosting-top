 ...
 * Поддерживает символы, допустимые в доменах: буквы, цифры, дефисы и точки
...
export function generateSlug(text: string, maxLength: number = 100): string {
  ...
  // Разрешаем символы, допустимые в доменах: буквы, цифры, дефисы, точки
  // Удаляем все остальные символы, кроме пробелов и подчеркиваний (их заменим позже)
  slug = slug.replace(/[^a-z0-9.\s_-]/g, "");

  // Замена пробелов и подчеркиваний на дефисы
  slug = slug.replace(/[\s_]+/g, "-");

  // Удаление повторяющихся дефисов (но не точек)
  slug = slug.replace(/-+/g, "-");

  // Удаление точек в начале и конце (точки не могут быть в начале/конце домена)
  slug = slug.replace(/^\.+|\.+$/g, "");

  // Удаление дефисов в начале и конце
  slug = slug.replace(/^-+|-+$/g, "");

  // Удаление двух и более точек подряд (домены не могут содержать две точки подряд)
  slug = slug.replace(/\.{2,}/g, ".");

  // Обрезка до максимальной длины
  if (slug.length > maxLength) {
    slug = slug.substring(0, maxLength);
    // Удаляем дефис или точку в конце, если они остались после обрезки
    slug = slug.replace(/[-.]+$/, "");
  }
